QUnit.module("Underscore Silent", {
    teardown: function () {
        var globj = _.getGlobalObject();
...
test("url", function () {
    ...
    if (_.isUndefined(globj.location)) {
        globj.location = {
            protocol : 'http',
            host : 'localhost'
        };
...
test("require", function () {
    ...
    throws(
        function () {
            _.require('Foo', function (Foo) {});
        },
        "Foo object does not exist."
    );
    ...
    deepEqual(_.require(['Foo'], function (Foo) {
        return Foo;
    }), Foo);
    ...
    deepEqual(_.require(['Foo', 'Bar'], function (Foo, Bar) {
        return [Foo, Bar];
    }), [Foo, Bar]);
